...
public class CassandraDeviceDao extends CassandraAbstractSearchTextDao<DeviceEntity, Device> implements DeviceDao {
    ...
    public Device save(TenantId tenantId, Device domain) {
        System.out.println("====================");
        System.out.println("device1="+domain);
    ...
    @Override
    public UUID findByCustomerId(UUID tenantId, UUID deviceId) {
        Select select = select().from(getColumnFamilyName());
        Select.Where query = select.where();
        query.and(eq(DEVICE_TENANT_ID_PROPERTY, tenantId));
        query.and(in(ID_PROPERTY, deviceId));
        DeviceEntity deviceEntity = findOneByStatement(new TenantId(tenantId), query);
        UUID customerId = deviceEntity.getCustomerId();
        return customerId;
    }
