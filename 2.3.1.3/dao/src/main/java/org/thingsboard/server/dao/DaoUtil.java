...
public abstract class DaoUtil {
    ...
    public static List<AlarmRecipient> convertDataList1(List<AlarmRecipient> toDataList) {
        System.out.println("toDataList="+toDataList);
        List<AlarmRecipient> list = new ArrayList<>();
        if (toDataList != null && !toDataList.isEmpty()) {
            for (AlarmRecipient alarmRecipient : toDataList) {
                list.add(alarmRecipient);
            }
        }
        return list;
    }
