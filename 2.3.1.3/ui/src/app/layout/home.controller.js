...
import logoKKT from  '../../svg/logo_kkt.svg';
import logoSTE from '../../svg/logo_ste.svg';
import logoJSH from '../../svg/logo_jsh.svg';
...
export default function HomeController(types, loginService, userService, deviceService, Fullscreen, $scope, $element, $rootScope, $document, $state,
    ...
    if(user && user.customerId === '983203d0-7867-11e9-947b-67b440b74b92') {
        vm.logoSvg = logoKKT;
    }
    if(user && user.customerId === '670021f0-942a-11e9-b6d9-99de8b6afd61') {
        vm.logoSvg = logoSTE;
    }
    if(user && user.customerId === 'fb027e40-abac-11e9-bf42-77b3c09eead2') {
        vm.logoSvg = logoJSH;
    }

    //for testing
    if(user && user.customerId === 'f6a7ec90-93d8-11e9-b592-b1a83fed10a9') {
        vm.logoSvg = logoKKT;
    }
    ...
    $scope.$on('$stateChangeSuccess', function (evt, to, toParams, from) {
        ...
        $log.info(to.name + ' ' +  from.name);

        //fixed the submenu state
        if(to.name === 'home.tdevice' || to.name === 'home.tparams'
            || to.name === 'home.tcurve' || to.name === 'home.tefficiency' || to.name === 'home.tlife') {
            angular.element('.tb-side-menu').attr('secondMenu', 'true');
        }

        if(to.name === 'home.devstatus' || to.name === 'home.oee' || to.name === 'home.maintenance' ) {
            angular.element('.tb-side-menu').attr('secondMenu', 'false');
        }
